...
** Last update	Sun Jul 02 11:05:54 2017 Gauthier Harnisch
*/
...
void	take(t_serv *serv, int i, char **tab)
{
  ...
  j = -1;
  while (++j < 7)
    if (match(tab[1], ressources[j]) && serv->cli[i]->pos->ressources[j] > 0)
      {
        serv->cli[i]->inventory[j] += 1;
        serv->cli[i]->pos->ressources[j] -= 1;
        map_refresh(serv->map, j, serv->sizex, serv->sizey);
